...
<center><font size='6'>硬件时钟和jiffies</font></center>
<br/>
<br/>
...
4. static struct clocksource refined_jiffies;
函数调用关系
1. 初始化
------------------------------------------------------- x86_64_start_kernel() x86_64_start_reservations() ... x86_intel_mid_early_setup() ...
...
# 2. Initialization of non-standard PC hardware clock
	   			  ...
	   			  ->do_basic_setup()
	   				->do_initcalls()
	   				  ->do_initcall_level()
	   					->do_one_initcall()
	   					  ->xxx__initcall()
        kernel_thread() kthreadd() => PID=2 cpu_startup_entry() while (1) do_idle(); prevent_tail_call_optimization()
```
...
> 但是在5.10.13中该函数指针是指向函数`x86_wallclock_init`的，如下定义：
...
![HZ](_v_images/20210330085314508_32588.png)
